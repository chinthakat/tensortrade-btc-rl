...
def test_environment_creation():
    ...
    try:
        ...
        print(f"\nChecking technical indicators in env.df:")
        print(f"Columns: {env.df.columns.tolist()}")
        
        # Check for NaN values in key columns
        key_columns = ['bb_upper', 'bb_middle', 'bb_lower', 'sma_20', 'returns']
        for col in key_columns:
            if col in env.df.columns:
                null_count = env.df[col].isnull().sum()
                print(f"{col}: {null_count} null values")
                if null_count > 0:
                    print(f"  First few values: {env.df[col].head(25).tolist()}")
        ...
        # Now test with the wrapped environment (without algo_type parameter)
        print("\n\nTesting with wrapped environment...")
        wrapped_env = wrap_environment_for_algorithm(env)
