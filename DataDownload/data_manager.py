...
class DataManager:
    ...
    def _download_from_coinapi_with_daily_batches(self, downloader, symbol: str, interval: str) -> Optional[pd.DataFrame]:
        """Download comprehensive market data from CoinAPI using daily batch method with funding rates and open interest"""
        try:
            use_date_range = self.config.get('use_date_range', False)
            
            # Determine which data types to download
            data_types = ['ohlcv']  # Always download OHLCV
            
            # Add funding rates and open interest if enabled
            if self.config.get('include_funding_rates', True):
                data_types.append('funding_rates')
            if self.config.get('include_open_interest', True):
                data_types.append('open_interest')
            
            self.logger.info(f"Downloading data types: {', '.join(data_types)}")
            ...
            if use_date_range:
                ...
                # Use daily batch download with multiple data types
                ...
                if success:
                    # Combine the multi-data types into a single DataFrame
                    merged_df = downloader.merge_multi_data_types(
                        symbol=symbol,
                        interval=interval,
                        start_date=start_date,
                        end_date=end_date
                    ...
                    if merged_df is not None:
                        self.logger.info(f"Successfully downloaded and merged {len(merged_df)} records")
                        return merged_df
                    else:
                        self.logger.error("Failed to merge multi-data types")
                        return None
                else:
                    self.logger.error("Daily batch download failed")
                    return None
            else:
                # Use recent data
                ...
                if success:
                    ...
                    self.logger.error("Recent data download failed")
                    return None
                    
        except Exception as e:
            self.logger.error(f"Multi-data CoinAPI download failed: {e}")
            return None
    
    def get_available_data_summary(self, symbol: str, interval: str = '15m') -> Dict[str, Any]:
        """Get summary of available data for a symbol"""
        return self.coinapi_downloader.get_available_data_summary(symbol, interval)
    
    def download_additional_data_types(self, symbol: str, interval: str = '15m', 
                                     data_types: List[str] = None, days: int = 30) -> bool:
        """Download additional data types for existing symbol data"""
        if data_types is None:
            data_types = ['funding_rates', 'open_interest']
        
        from datetime import datetime, timedelta
        end_date = datetime.now() - timedelta(days=1)
        start_date = end_date - timedelta(days=days)
        
        return self.coinapi_downloader.download_daily_data_batch(
            symbol=symbol,
            interval=interval,
            start_date=start_date.strftime('%Y-%m-%d'),
            end_date=end_date.strftime('%Y-%m-%d'),
    ...
    def load_multi_data_combined(self, symbol: str, interval: str = '15m', 
                               days: int = None) -> Optional[pd.DataFrame]:
        """Load combined multi-data type DataFrame"""
        start_date = None
        end_date = None
        ...
        return self.coinapi_downloader.merge_multi_data_types(
            symbol=symbol, 
            interval=interval, 
            start_date=start_date, 
            end_date=end_date
    ...
    def _load_and_merge_multi_data(self, downloader, symbol: str, interval: str, 
                                  start_date: str, end_date: str, data_types: List[str]) -> Optional[pd.DataFrame]:
        """Load and merge multiple data types (OHLCV, funding rates, open interest) from saved files"""
        try:
            # Find data directory
            data_dir = self._find_data_directory(downloader)
            if data_dir is None:
            ...
            for data_type in data_types:
                data_type_dir = base_dir / data_type
                
                if not data_type_dir.exists():
                    self.logger.warning(f"No {data_type} data directory found: {data_type_dir}")
                    continue
                
                # Get CSV files for this data type
                csv_files = list(data_type_dir.glob("*.csv"))
                if not csv_files:
                    self.logger.warning(f"No CSV files found for {data_type} in {data_type_dir}")
                    continue
                
                # Filter relevant files by date range
                relevant_files = self._filter_relevant_files(csv_files, symbol, interval, start_date, end_date)
                
                if not relevant_files:
                    self.logger.warning(f"No relevant {data_type} files found for date range")
                    continue
                
                # Load and combine files for this data type
                df_data_type = self._load_and_combine_files(relevant_files, start_date, interval)
                
                if df_data_type is not None and len(df_data_type) > 0:
                    data_frames[data_type] = df_data_type
                    self.logger.info(f"Loaded {len(df_data_type)} {data_type} records")
                    print(f"📊 Loaded {data_type}: {len(df_data_type)} records")
                else:
                    self.logger.warning(f"Failed to load {data_type} data")
                    print(f"⚠️  Failed to load {data_type} data")
            
            # Must have at least OHLCV data
            if 'ohlcv' not in data_frames:
                self.logger.error("No OHLCV data found - cannot proceed")
                return None
            
            # Start with OHLCV as base
            merged_df = data_frames['ohlcv'].copy()
            print(f"📊 Base OHLCV data: {len(merged_df)} records")
            
            # Merge other data types on timestamp
            for data_type in ['funding_rates', 'open_interest']:
                if data_type in data_frames:
                    other_df = data_frames[data_type]
                    
                    # Merge on index (timestamp)
                    merged_df = merged_df.join(other_df, how='left', rsuffix=f'_{data_type}')
                    
                    # Count successful merges
                    non_null_count = merged_df[[col for col in merged_df.columns if data_type.replace('_', '') in col.lower()]].count().sum()
                    print(f"📊 Merged {data_type}: {non_null_count} matched records")
                    self.logger.info(f"Merged {data_type}: {non_null_count} non-null values after join")
            
            # Forward fill funding rates and open interest (they change less frequently)
            funding_cols = [col for col in merged_df.columns if 'funding' in col.lower()]
            oi_cols = [col for col in merged_df.columns if 'open_interest' in col.lower()]
            
            for col in funding_cols + oi_cols:
                merged_df[col] = merged_df[col].fillna(method='ffill')
            
            # Fix timestamp issues
            merged_df = self._fix_timestamp_issues(merged_df)
            
            self.logger.info(f"Successfully merged multi-data: {len(merged_df)} records with {len(merged_df.columns)} columns")
            ...
            self.logger.error(f"Error merging multi-data: {e}")
            return None
